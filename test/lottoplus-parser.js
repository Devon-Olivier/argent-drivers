...
const parse = require('../src/lottoplus/parser');
...
describe('parse', () => {
  ...
  drawHtmlArray.forEach((html, i) => {
    it(`should give parse draw number ${expectedDraws[i].drawNumber} correctly`,
      () => {
        const draw = parse(html);
        assert.deepStrictEqual(draw, expectedDraws[i]);
      });
  });
});
